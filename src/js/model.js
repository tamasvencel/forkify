...
export const loadSearchResults = async function (query) {
  try {
    state.search.query = query; // &key because we already have a "?"
    const data = await AJAX(
...
export const getSearchResultsPage = function (page = state.search.page) {
  ...
  return state.search.results.slice(start, end); // the slice method does not include the last value
};
  ...
  try {
    const ingredients = Object.entries(newRecipe)
      ...
      .map((ing) => {
        ...
        if (ingArr.length !== 3)
          throw new Error(
            "Wrong ingredient format! Please use the correct format :) Please reload the page if you want to add more recipe!"
          );
